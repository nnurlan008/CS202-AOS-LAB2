...
found:
  p->pid = allocpid();
  p->state = USED;
  p->tickets_value = 1000; // lab2: initial tickets value
  p->ticks_number = 0; // lab2: initial ticks number
  p->stride_value = K / p->tickets_value; // lab2: initial stride value
  p->pass_value = p->stride_value; // lab2: initial pass value
...
{  
  ...
  for (;;) {
    ...
    // here we calculate the number of total_tickets
    for (p = proc; p < &proc[NPROC]; p++) { // iterate on processors 
      acquire(&p->lock); // lock

      if (p->state == RUNNABLE) total_tickets = total_tickets + p->tickets_value;

      release(&p->lock); // release lock
    }
    int flag = 0;
    int tickets_count = 0;
    // finding winner from range inside total_tickets
    winner = rand() % total_tickets;
    for (p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock); // lock

      if (p->state == RUNNABLE) {
          tickets_count = tickets_count + p->tickets_value;
          if(tickets_count >= winner){
            p->state = RUNNING;
            flag = flag + 1;
            p->ticks_number = p->ticks_number + 1;
...
scheduler(void)
{
  ...
  for (;;) {
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      ...
      if (p->state == RUNNABLE && p->pass_value < min_pass) min_pass = p->pass_value;
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      ...
      if (p->state == RUNNABLE && p->pass_value == min_pass) {
          ...
          p->pass_value = p-> pass_value + p->stride_value;
          p->ticks_number = p->ticks_number + 1;
...
void proc_sched_statistics(){
  struct proc *p;
    for (p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock); // lock

      if (p->state != UNUSED){ //1(init): tickets: xxx, ticks: 21
          printf("%d(%s): ",p->pid,p->name);
          printf("tickets: %d, ", p->tickets_value);
          printf("Ticks: %d\n", p->ticks_number);
      }
...
int proc_set_sched_tickets(int ticket){ 
  struct proc *p = myproc();
  if (ticket < 0)
    ticket = 1;
  else if (ticket > 10000)
    ticket = 10000;
  
  p->tickets_value = ticket;
  p->stride_value = K / ticket; // K = 10000 return 0;
  return 0;
}
